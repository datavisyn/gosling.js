export type GoslingSpec = (RootSpecWithSingleView & ResponsiveSpecOfSingleView) | (RootSpecWithMultipleViews & ResponsiveSpecOfMultipleViews);
export type ResponsiveSize = boolean | {
    width?: boolean;
    height?: boolean;
};
 ...
 */
export interface DummyTrack extends Pick<CommonTrackDef, 'width' | 'height' | 'id' | 'title' | '_invalidTrack' | 'overlayOnPreviousTrack' | 'orientation' | 'layout' | 'static' | 'assembly'> {
...
export type Mark = 'point' | 'line' | 'area' | 'bar' | 'rect' | 'text' | 'withinLink' | 'betweenLink' | 'rule' | 'triangleLeft' | 'triangleRight' | 'triangleBottom' | 'brush' | 'header';
...
export type LogicalOperation = 'less-than' | 'lt' | 'LT' | 'greater-than' | 'gt' | 'GT' | 'less-than-or-equal-to' | 'ltet' | 'LTET' | 'greater-than-or-equal-to' | 'gtet' | 'GTET';
...
export type DataDeep = JsonData | CsvData | BedData | BigWigData | MultivecData | BeddbData | VectorData | MatrixData | BamData | VcfData | GffData;
...
export type DataTransform = FilterTransform | StrConcatTransform | StrReplaceTransform | LogTransform | DisplaceTransform | ExonSplitTransform | GenomicLengthTransform | SvTypeTransform | CoverageTransform | JsonParseTransform;
...
export type DataTransformWithBase = Partial<DataTransform> & {
    base?: string;
};
/**
 ...
 */
export type TemplateTrackMappingDef = Omit<CommonTrackDef, 'data' | 'height' | 'width' | 'layout' | 'title' | 'subtitle'> & {
